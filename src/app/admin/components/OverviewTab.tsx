  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-dark-400 mt-1">Here&apos;s what&apos;s happening with your store today</p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="bg-dark-900/60 border border-l-4 border-l-blue-400/50 border-dark-800/50 rounded-2xl">
            ...
            {[
              ...
              >
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${
                  item.type === "order" ? "bg-blue-500/10 border border-blue-500/20" :
                  item.type === "security" ? "bg-red-500/10 border border-red-500/20" :
                  "bg-gold-500/10 border border-gold-500/20"
                }`}>
                  {item.type === "order" ? <Package size={14} className="text-blue-400" /> :
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-xs text-dark-300">
                    ...
                    {item.type === "security" && <><span className="text-white font-medium group-hover:text-gold-400 transition-colors">{item.data.user?.name || "User"}</span> — password reset <span className={`font-medium ${item.data.status === "completed" ? "text-green-400" : item.data.status === "failed" ? "text-red-400" : "text-yellow-400"}`}>{item.data.status}</span></>}
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-3 text-center">
              <p className="text-xl font-display font-bold text-yellow-400">{messages?.unread || 0}</p>
              <p className="text-xs text-dark-500">Unread</p>
            </div>
            <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-3 text-center">
              <p className="text-xl font-display font-bold text-green-400">{(messages?.messages || []).filter((m: any) => m.status === "replied").length}</p>
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-3 text-center">
              <p className="text-xl font-display font-bold text-green-400">{passwordResets.filter((r: any) => r.status === "completed").length}</p>
              <p className="text-xs text-dark-500">Completed</p>
            </div>
            <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-3 text-center">
              <p className="text-xl font-display font-bold text-yellow-400">{passwordResets.filter((r: any) => r.status === "requested").length}</p>

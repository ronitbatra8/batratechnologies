...
export default function DeliveryExecTab({ adminKey }: { adminKey: string }) {
  ...
  const categories = [
    ...
    { key: "approved", label: "Approved", count: approved, color: "text-green-400" },
    { key: "pending", label: "Pending", count: pending, color: "text-yellow-400" },
    { key: "active", label: "Active Orders", count: withOrders, color: "text-purple-400" },
  ];
              ...
              >
                ...
                >
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 bg-gradient-to-br ${statusGrad} border border-dark-700/50`}>
                    {exec.approved ? <UserCheck className="w-5 h-5 text-green-400" /> : <UserX className="w-5 h-5 text-yellow-400" />}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                      ...
                      {exec.approved ? (
                        <span className="inline-block px-2.5 py-0.5 rounded-full text-xs font-semibold w-fit bg-green-500/15 text-green-400">Approved</span>
                      ) : (
                        <span className="inline-block px-2.5 py-0.5 rounded-full text-xs font-semibold w-fit bg-yellow-500/15 text-yellow-400">Pending</span>
                      )}
                    </div>

...
export default function EditProduct() {
  return <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="w-10 h-10 border-2 border-gold-500/20 border-t-gold-500 rounded-full animate-spin" /></div>}><EditProductInner /></Suspense>;
}
...
function EditProductInner() {
  ...
  useEffect(() => {
    ...
    if (!id) return;
    apiFetch(`/seller/products/${id}`).then(p => {
      if (p.sellerId !== user.id) { router.push("/seller"); return; }
      setForm({ name: p.name, brand: p.brand, category: p.category, price: String(p.price), originalPrice: p.originalPrice ? String(p.originalPrice) : "", description: p.description, features: (p.features || []).join("\n"), images: (p.images || []).join("\n"), inStock: p.inStock, badge: p.badge || "" });
    }).catch(() => router.push("/seller")).finally(() => setFetching(false));
  ...
  if (fetching) return <div className="min-h-screen flex items-center justify-center"><div className="w-10 h-10 border-2 border-gold-500/20 border-t-gold-500 rounded-full animate-spin" /></div>;
    ...
    <div className="min-h-screen bg-dark-950 page-transition">
      <div className="max-w-2xl mx-auto px-6 pt-28 pb-24">
        ...
        <form onSubmit={handleSubmit} className="bg-dark-900/60 border border-dark-800/50 rounded-2xl p-8 space-y-5">
          ...
          <div><label className="block text-xs text-dark-400 uppercase tracking-wider mb-2">Images (one URL per line)</label><textarea value={form.images} onChange={e => setForm({...form, images: e.target.value})} rows={3} className="w-full bg-dark-800 border border-dark-700 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-gold-500 transition-colors resize-none" /></div>
          ...
          <button type="submit" disabled={saving} className="w-full bg-gradient-to-r from-gold-500 to-gold-600 hover:from-gold-400 hover:to-gold-500 text-dark-950 py-4 rounded-xl font-semibold transition-all disabled:opacity-50 flex items-center justify-center gap-2"><Save size={16} /> {saving ? "Saving..." : "Save Changes"}</button>
        </form>
      </div>
    </div>
  );

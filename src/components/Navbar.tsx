...
export default function Navbar() {
  ...
  return (
    <>
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "top-3 left-4 right-4 rounded-full py-4 bg-dark-950/55 backdrop-blur-2xl backdrop-saturate-150 border border-white/10 shadow-lg shadow-black/40 lg:top-0 lg:left-0 lg:right-0 lg:rounded-none lg:py-3 lg:bg-dark-950/90 lg:border-b lg:border-x-0 lg:border-t-0 lg:shadow-none" : "py-5 bg-transparent lg:py-5"}`}>
    ...
    <nav className={`fixed bottom-4 left-4 right-4 z-50 lg:hidden bg-dark-950/55 backdrop-blur-2xl backdrop-saturate-150 rounded-full border border-white/10 shadow-lg shadow-black/40 pb-[env(safe-area-inset-bottom)] transition-transform duration-300 ${hideNav ? "translate-y-[calc(100%+1rem)]" : "translate-y-0"}`}>
      <div className="grid grid-cols-4">
        {[
          ...
          return (
            <Link key={item.label} href={item.href} className={`flex flex-col items-center gap-1 py-4 text-[10px] uppercase tracking-widest font-medium transition-colors ${item.active ? "text-gold-400" : "text-dark-400 hover:text-gold-400"}`}>
              <Icon size={20} className={item.active ? "text-gold-400" : ""} />
              {item.label}
            </Link>
          );
        })}
